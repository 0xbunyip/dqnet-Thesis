...
\subsubsection*{Lý do cần áp dụng học sâu}
	Thuật toán ``Q-learning'' được trình bày trong chương trước tìm chính sách tối ưu dựa vào việc tìm hàm giá trị tối ưu. 
	Việc tính đúng và nhanh hàm giá trị ảnh hưởng rất nhiều đến kết quả của bài toán. 
	...
	Còn về mặt tốc độ tính toán thì thuật toán ``Q-learning'' được trình bày ở chương 2 tính hàm giá trị một cách \textbf{rời rạc} cho từng trạng thái.
...
\subsection{Sử dụng mạng nơ-ron để xấp xỉ hàm}
	Việc sử dụng mạng nơ-ron tích chập (CNN) để xấp xỉ hàm giá trị mang lại ba lợi ích quan trọng:
	\begin{itemize}
		\item CNN có khả năng học được những đặc trưng phức tạp từ dữ liệu thô.
		\item CNN có thể xấp xỉ hàm giá trị phức tạp.
		\item CNN có thể tổng quát hoá từ trạng thái đã gặp sang trạng thái chưa gặp.
	\end{itemize}
	...
	Một tính chất quan trọng khác của CNN chính là khả năng tổng quát hoá (generalization) từ trạng thái đã gặp sang trạng thái chưa gặp \cite{mnihdqn2015}.
	Nhờ đặc điểm này mà quá trình học được tăng tốc đáng kể.
	Thay vì phải duyệt qua từng trạng thái (thậm chí phải duyệt nhiều lần) để tính hàm giá trị tại đó, mạng nơ-ron tích chập có khả năng \textbf{``dự đoán'' giá trị} của một trạng thái chưa từng thấy dựa vào những trạng thái đã thấy.
	...
	Để sử dụng CNN như một công cụ xấp xỉ hàm, lúc này ta coi hàm giá trị là một hàm có tham số (parameterized function) và đi tìm các tham số này:
	\begin{align}
	...
	Kỳ vọng $\mathbb{E}_{s, \sim \pi}$ ý chỉ kỳ vọng với biến ngẫu nhiên là trạng thái $s$ và hành động $a$ được lấy bằng cách tuân theo $\pi$.
	...
	Tuy nhiên ta không thể tính độ lỗi trên mọi điểm dữ liệu đầu vào theo công thức (\ref{eq_ExpectedErrorQ}) được vì số lượng trạng thái có thể có là rất lớn.
	...
	Áp dụng thuật toán SGD để cực tiểu hoá hàm lỗi (\ref{eq_BatchErrorQ}), công thức cập nhật bộ trọng số tại thời điểm $t$ có dạng:
	\begin{align}
...
\subsection{Học tăng cường kết hợp với xấp xỉ hàm}
	...
	Để sử dụng CNN để xấp xỉ hàm giá trị cho thuật toán ``Q-learning'', ta thực hiện việc lấy mẫu giá trị điểm thưởng để ước lượng $q_{*}(s,a)$.
...
\subsubsection*{Cố định ``Q-target''}
	...
	Do giá trị mục tiêu thay đổi quá nhiều và liên tục, CNN có thể không xấp xỉ được hàm giá trị như ta mong muốn.
	...
	Nếu tần suất quá thấp (tức ít khi thay đổi giá trị đích) thì việc học sẽ rất chậm do ta phải cập nhật trọng số $\theta$ rất nhiều để xấp xỉ một hàm giá trị cũ; lúc này, ta tốn rất nhiều công sức chỉ để xấp xỉ một giá trị đích không chính xác.
